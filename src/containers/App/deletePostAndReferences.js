...
export const deletePostAndReferences = ({ posts, client }) => async ({ id }) => {
  ...
  if (post?.document) { /* eslint-disable-line */
    if (post.document?.versions?.length) {  /* eslint-disable-line */
      await Promise.all(
        post.document.versions.map(deleteVersion({ client }))  /* eslint-disable-line */
      )
    }
